...
DS.IndexedDBAdapter = DS.Adapter.extend({
  ...
  /**
	 ...
	 * @param {DS.Model}
	 *            type
	 * @param {Object|String|Integer|null}
	 *            id
	 * @param {Object|null}
	 *            opts
	 */
  ...
  /**
	 ...
	 *            store
	 * @param {DS.Model}
	 *            type the model that we're retrieving
	 * @param {Array}
	 *            ids ids of the records we want to be returned.
	 */
  ...
  /**
	 ...
	 *            type the model
	 * @param {Object}
	 *            query object with fields we want to look for
	 * @param {Array}
	 *            recordArray
	 */
          ...
          Em.run(function() {
            ...
            if (cursor) {
              for (var field in query) {
                ...
                /**
				 * If it was already defined that the current record doesn't
				 * match the query, leave the search.
				 */
  ...
  /**
	 ...
	 * For example,
	 * 
	 * ```js store.createRecord('user', {name: "Rambo"}) ```
	 * 
	 * @method createRecord
	 * @param {DS.Store}
	 *            store
  ...
  /**
	 * Generates a random number. You will usually want to implement UUID in
	 * your app and redefine this method.
	 * 
  ...
  /**
	 ...
	 * 
	 * ```js { "id": 1, "title": "Rails Rambo", "comments": [1, 2] }
	 * 
	 * This will return:
	 * 
	 * ```js { "id": 1, "title": "Rails Rambo", "comments": [1, 2]
	 * 
	 * "_embedded": { "comment": [{ "_id": 1, "comment_title": "FIRST" }, {
	 * "_id": 2, "comment_title": "Rails is unagi" }] } }
	 * 
  ...
  loadRelationships: function(type, record) {
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      ...
      relationships.forEach(function(relationName) {
		 ...
		 * payload, such as:
		 *  { cart: { id: "s85fb", customer: "rld9u" } }
		 * 
		 * In this case, cart belongsTo customer and its id is present in the
		 * main payload. We find each of these records and add them to
		 * _embedded.
		 */
  ...
  /**
	 ...
	 *  { cart: { id: "1", customer: "2" }, _embedded: { customer: { id: "2",
	 * name: "Rambo" } } }
	 * 
  ...
  loadRelationshipsForMany: function(type, recordsArray) {
    ...
    return new Ember.RSVP.Promise(function(resolve, reject) {
      ...
      /**
		 * Some times Ember puts some stuff in arrays. We want to clean it so we
		 * know exactly what to iterate over.
		 */
