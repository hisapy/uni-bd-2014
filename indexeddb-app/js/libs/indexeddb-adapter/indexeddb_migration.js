...
DS.IndexedDBMigration = Ember.Object.extend({
	...
	migrate : function() {
		...
		return new Ember.RSVP.Promise(function(resolve, reject) {
			...
			connection.onupgradeneeded = function(event) {
				Em.run(function() {
					_this.set('memoizedOpenDatabaseForUpgrade',
							event.target.result);
					_this.runMigrations();
			...
			connection.onerror = function(e) {
				Em.run(function() {
					console.log('Failure connecting to IndexedDB database '
							+ _this.databaseName);
					reject(e);
				})
			}
	 ...
	 * 
	 * App.ApplicationAdapter = DS.IndexedDBAdapter.extend({ databaseName:
	 * 'some_database_name' version: 1, migrations: function() {
	 * this.addModel(App.Person); this.addModel(App.Phone); } });
	 * 
	...
	addModel : function(model, opts) {
		var db = this.memoizedOpenDatabaseForUpgrade, modelName = model
				.toString(), opts = opts || {}, _this = this;
	...
	currentDbVersion : function() {
		var instance = indexedDB.open(this.databaseName), version;
