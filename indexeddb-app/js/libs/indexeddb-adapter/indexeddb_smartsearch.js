 ...
 */
DS.IndexedDBSmartSearch = Ember.Object
		.extend({
			...
			/**
			 * The entrypoint. It tries to match the current query field against
			 * the record. See below each query explained.
			 *  == Search ==
			 * 
			 * store.findQuery('person', {search: "rao"})
			 * 
			 * This will will search for any field that has the string rao, such
			 * as "Rambo".
			 *  == Search ==
			 * 
			 * store.findQuery('person', {createdAt: "32 days ago"})
			 * 
			 * Given `createdAt` field has a transform type `date`, it will
			 * returns only records that match the 32nd day ago.
			 * 
			 * If the fields doesn't have the `date` transform, nothing is
			 * queried.
			 * 
			 * Besides `x days ago`, `today` and `yesterday` are also accepted.
			 */
			isMatch : function() {
				var record = this.get('record'), type = this.get('type'), field = this
						.get('field'), queryString = this.get('queryString'), fieldType = this
						.fieldType(field), queryType;
			...
			searchField : function(type, record, field, queryString) {
				...
				for ( var queriedField in record) {
					var isSearchField = this.get('fieldSearchCriteria')(
							queriedField, type), fieldValue = record[queriedField];
					...
					if (Object.prototype.toString.call(queryString).match(
							"RegExp")) {
						isMatch = isMatch
								|| new RegExp(queryString).test(fieldValue);
					} else {
			...
			dateField : function(type, record, field, queryString) {
				var rawValue = record[field], date = (new Date(Date
						.parse(rawValue))), targetDate = new Date(), match;

				var IsMatchToDate = function(targetDate) {
					var year = targetDate.getFullYear(), month = targetDate
							.getMonth(), day = targetDate.getDate(), hour = targetDate
							.getHours(), minute = targetDate.getMinutes();
			...
			fieldType : function(fieldName) {
				if (fieldName === "search") {
					...
					var type = this.get('type'), transform;
